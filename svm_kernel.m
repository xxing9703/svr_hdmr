function varargout= svm_kernel(ker,u,v,para)
%hdmrkernel for SVR
%  Usage: k = svm_kernel(ker,u,v,para)
%      or [k,k1]=svm_kernel(ker,u,v,para)
%      or [k,k1,kall]=svm_kernel(ker,u,v,para)
%      or [k,k1,k2,kall]=svm_kernel(ker,u,v,para)
...
switch lower(ker) 
 ...
 if(nargout==3)
     kall=e.*sig';  % sum of a given order
     varargout{3}=kall;       
 end
 if(nargout==4)
     ...
     for i=1:s2
     for j=i+1:s2
        k2(count)=kbase(i)*kbase(j)*sig(2); %2nd order 
     count=count+1;
     end
     end
      varargout{3}=k2; 
